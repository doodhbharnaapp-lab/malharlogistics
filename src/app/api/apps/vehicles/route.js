...
export async function GET(req) {
    try {
        ...
        const vehicleNo = searchParams.get('vehicleNo')
        const db = await getDB()
        /* ===== VEHICLE TYPES - Anyone with READ can access ===== */
        if (action === 'types') {
            // Check if user has vehicles:read permission
            ...
            if (!hasPermission && session.user.role !== 'admin') {
                return NextResponse.json(
                    { error: 'Forbidden: You need vehicles:read permission' },
                    { status: 403 }
                )
            }
            const vehicleTypes = await db.collection('vehicleTypes')
...
/* ===========================
   POST – Create Vehicle
=========================== */
export const POST = checkPermission(PERMISSIONS.vehicles.CREATE)(
    async function POST(req) {
        try {
            ...
            const vehicle = {
                ...
                ownerId: new ObjectId(data.ownerId),
                driverId: data.driverId ? new ObjectId(data.driverId) : null,
                manualDriver: data.manualDriver || null,
                model: data.vehicleModel,
                bankName: data.bankName || '',
                accountNo: data.accountNo || '',
                ifscCode: data.ifscCode || '',
...
/* ===========================
   PUT – Update Vehicle
=========================== */
export const PUT = checkPermission(PERMISSIONS.vehicles.WRITE)(
    async function PUT(req) {
        try {
            ...
            // Check if this is a bulk operation
            if (data.ids && Array.isArray(data.ids)) {
                // BULK STATUS UPDATE - requires WRITE permission
                const { ids, isActive } = data
                if (!ids || ids.length === 0) {
                    return NextResponse.json(
                        { error: 'Vehicle IDs are required' },
                ...
                // Check if new vehicle number conflicts
                if (data.vehicleNo !== existingVehicle.vehicleNo) {
                    const duplicate = await db.collection('vehicles').findOne({
                        vehicleNo: { $regex: new RegExp(`^${data.vehicleNo}$`, 'i') },
                        _id: { $ne: vehicleId },
...
=========================== */
export const PATCH = checkPermission(PERMISSIONS.vehicles.WRITE)(
...
/* ===========================
   DELETE – Single or Bulk Delete
=========================== */
export const DELETE = checkPermission(PERMISSIONS.vehicles.DELETE)(
