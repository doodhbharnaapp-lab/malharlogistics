...
//             const finalRouteCode = routeCode || (selectedRoute?.routeCode) || '';
//             const finalDistanceKm = distanceKm || (selectedRoute?.distanceKm) || 0;
//             // Check 1: Check if trip already exists for the same vehicle and date
//             const existingTripOnDate = await db.collection(TRIPS_COLLECTION).findOne({
//                 vehicleNo: vehicleNo.trim(),
//                 tripDate: tripDate || new Date().toISOString().split('T')[0],
//                 isDeleted: { $ne: true }
...
//                 )
//             }
//             // Check 2: Count ONLY ACTIVE trips for this vehicle
//             const activeTripsForVehicle = await db.collection(TRIPS_COLLECTION).countDocuments({
//                 vehicleNo: vehicleNo.trim(),
//                 tripStatus: 'active',
//                 isDeleted: { $ne: true }
...
// )
// /* ================= PUT - UPDATE TRIP ================= */
// export const PUT = checkPermission(PERMISSIONS.marketTrips.WRITE)(
...
//                         PERMISSIONS.marketTrips.BULK_UPDATE
//                     ])(async () => true)(req)
//                     if (!hasBulkPermission) {
//                         return NextResponse.json({
//                             error: 'Forbidden',
//                             message: 'You need marketTrips:bulk-update permission for bulk operations'
//                         }, { status: 403 })
//                     }
//                 }
...
// export const PATCH = checkPermission(PERMISSIONS.marketTrips.WRITE)(
...
// )
// /* ================= DELETE TRIP (SOFT DELETE) ================= */
// export const DELETE = checkPermission(PERMISSIONS.marketTrips.DELETE)(
...
// )
// /* ================= BULK DELETE TRIPS ================= */
// export const POST_BULK_DELETE = checkPermission(PERMISSIONS.marketTrips.BULK_DELETE)(
...
// )
// /* ================= BULK STATUS UPDATE ================= */
// export const POST_BULK_STATUS = checkPermission(PERMISSIONS.marketTrips.BULK_STATUS)(
...
export const POST = checkPermission(PERMISSIONS.marketTrips.CREATE)(
    async function POST(req) {
        try {
            ...
            // Validate required fields - UPDATED: removed diesel fields, added from/to
            if (!vehicleNo || !driverName || !fromLocation || !toLocation) {
                return NextResponse.json(
                    {
                        success: false,
            ...
            // UPDATED: Removed all diesel fields, added from/to locations
            const payload = {
                vehicleNo: vehicleNo.trim(),
                driverName: driverName.trim(),
                vehicleType: vehicleType || '',
