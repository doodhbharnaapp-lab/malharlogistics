...
export async function GET(request) {
    try {
        ...
        // ✅ CHECK PERMISSION - marketAdvance:read
        const hasPermission = await checkAnyPermission([
            PERMISSIONS.marketAdvance.READ,
            PERMISSIONS.marketAdvance.CREATE,
            PERMISSIONS.marketAdvance.WRITE
        ])(async () => true)(request)

        if (!hasPermission && session.user.role !== 'admin') {
            return NextResponse.json({
                error: 'Forbidden',
                message: 'You need marketAdvance:read permission'
            }, { status: 403 })
        }
...
/* ================= CREATE ADVANCE ================= */
export const POST = checkPermission(PERMISSIONS.marketAdvance.CREATE)(
    async function POST(req) {
        try {
            ...
            // 🚨 SPECIAL CHECK FOR ACCOUNT TWO (if applicable to market advance)
            if (session.user.role === 'account-two') {
                const hasDieselOnly = await checkSinglePermission(
                    session.user.role,
                    PERMISSIONS.advance.DIESEL_ONLY // Use same permission for market advance if needed
                )

                if (hasDieselOnly && advanceType !== 'diesel') {
                    return NextResponse.json({
                        error: 'Forbidden',
                        message: 'Account Two can only create DIESEL advances'
                    }, { status: 403 })
                }
            }
            ...
            // Check advance limits for unpaid
            if (status === 'unpaid') {
                const unpaidAdvances = await db.collection(ADVANCES_COLLECTION).find({
                    tripId: new ObjectId(tripId),
                    status: 'unpaid',
            ...
            const advanceId = `MADV${(advanceCount + 1).toString().padStart(3, '0')}`
...
/* ================= UPDATE ADVANCE STATUS ================= */
export const PUT = checkPermission(PERMISSIONS.marketAdvance.WRITE)(
...
/* ================= DELETE ADVANCE ================= */
export const DELETE = checkPermission(PERMISSIONS.marketAdvance.DELETE)(
...
/* ================= BULK MARK AS PAID ================= */
export const POST_BULK_PAID = checkPermission(PERMISSIONS.marketAdvance.BULK_UPDATE)(
    async function POST_BULK_PAID(req) {
        try {
            const session = await getServerSession(authOptions)
            const { advanceIds } = await req.json()
