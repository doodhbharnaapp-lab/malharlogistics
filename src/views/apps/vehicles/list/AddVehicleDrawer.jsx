...
//     // New state for driver selection mode
//     const [driverMode, setDriverMode] = useState('select') // 'select' or 'manual'
//     // ===================== Form =====================
//     const {
//         control,
//         handleSubmit,
...
//     useEffect(() => {
//         if (watchDriverId && driverMode === 'select') {
...
//         }
//     }, [watchDriverId, drivers, driverMode])
...
//     useEffect(() => {
//         if (editData && open) {
//             console.log('Edit Data (full):', JSON.stringify(editData, null, 2))
...
//         }
//     }, [editData, open])
//     // Replace your entire useEffect for edit mode with this:
//     useEffect(() => {
...
//             }
//             // Check if this vehicle has manual driver data
//             // Since we have driverName and driverMobile directly in the response,
//             // and no driverId, this should be treated as manual mode
//             let driverModeValue = 'manual'; // Default to manual since we have driverName/driverMobile
//             let driverIdValue = '';
//             let manualDriverNameValue = editData.driverName || '';
//             let manualDriverMobileValue = editData.driverMobile || '';
//             // Only try to find driver in dropdown if we have a driverId field
//             // But from your logs, there's no driverId, so we stick with manual mode
//             console.log('Setting driver mode to manual with:', {
//                 name: manualDriverNameValue,
//                 mobile: manualDriverMobileValue
//             });
//             setDriverMode(driverModeValue);
//             // Reset the form with all values
//             const formValues = {
//                 vehicleNo: editData.vehicleNo || '',
//                 ownerId: ownerId,
//                 vehicleModel: editData.vehicleModel || editData.vehicleType || '',
//                 driverId: driverIdValue,
//                 manualDriverName: manualDriverNameValue,
//                 manualDriverMobile: manualDriverMobileValue,
...
//                 console.log('Setting documents:', editData.documents);
//                 setDocuments(
...
//             }
//         }
//     }, [editData, open, owners, drivers, types, reset]);// Add all dependencies
...
//             setSuccess('');
//         }
//     }, [editData, open, reset]); // Remove owners/drivers/types from dependencies
//     // ===================== Document Handlers =====================
//     const addDocument = () =>
...
//                     publicId: d.publicId || ''
//                 }))
//             // Prepare payload based on driver mode
//             const payload = {
//                 ...formData,
//                 documents: filteredDocuments,
//                 driverMode: driverMode
//             }
//             // Handle driver data based on mode
//             if (driverMode === 'manual') {
//                 // For manual mode, include manual driver details and remove driverId
//                 payload.manualDriver = {
//                     name: formData.manualDriverName,
//                     mobile: formData.manualDriverMobile,
//                 }
//                 delete payload.driverId
//             } else {
//                 // For select mode, include driverId and remove manual fields
//                 delete payload.manualDriverName
//                 delete payload.manualDriverMobile
//                 delete payload.manualDriver
//             }
...
//                                                             label="Driver Name"
//                                                             size="small"
//                                                             error={!!errors.manualDriverName}
//                                                             helperText={errors.manualDriverName?.message}
//                                                             disabled={isSubmitting}
//                                                         />
//                                                     )}
//                                                 />
...
//                                             <Grid item xs={12} sm={6}>
//                                                 <Controller
//                                                     name="manualDriverMobile"
//                                                     control={control}
//                                                     rules={{ required: 'Driver mobile is required' }}
//                                                     render={({ field }) => (
//                                                         <TextField
//                                                             {...field}
//                                                             fullWidth
//                                                             label="Driver Mobile"
//                                                             size="small"
//                                                             error={!!errors.manualDriverMobile}
//                                                             helperText={errors.manualDriverMobile?.message}
//                                                             disabled={isSubmitting}
//                                                         />
//                                                     )}
//                                                 />
...
const AddVehicleDrawer = ({
    ...
    const [success, setSuccess] = useState('')
    const [selectedDriverMobile, setSelectedDriverMobile] = useState('')
    const [driverMode, setDriverMode] = useState('select')
    ...
    // ===================== Handle Edit Data =====================
    useEffect(() => {
        if (editData && open && dataLoaded && owners.length > 0) {
            ...
            // Set driver mode to manual since we have driverName and driverMobile
            ...
            const formValues = {
                ...
                vehicleModel: editData.vehicleModel || editData.vehicleType || '',
                driverId: '',
                manualDriverName: editData.driverName || '',
                manualDriverMobile: editData.driverMobile || '',
                bankName: editData.bankName || '',
                accountNo: editData.accountNo || '',
                ifscCode: editData.ifscCode || '',
                accountHolderName: editData.accountHolderName || '',
    ...
    }, [editData, open, dataLoaded, owners, reset])
        ...
        >
                ...
                {isLoading ? (
                    ...
                    <>
                        {error && (
                            <Alert
                                severity="error"
                                sx={{ mb: 3, borderRadius: 1.5, fontSize: '0.875rem' }}
                                onClose={() => setError('')}
                            >
                                {error}
                            </Alert>
                                            ...
                                            {selectedDriverMobile && (
                                                <Grid size={{ xs: 12, sm: 6 }}>
                                                    <TextField
                                                        ...
                                                        InputProps={{
                                                            readOnly: true,
                                                            sx: { backgroundColor: 'grey.50', fontSize: '0.875rem' }
                                                        }}
                                                        size="small"
                                                    />
                                                </Grid>
                                                        ...
                                                        {doc.imageUrl ? (
                                                            <>
                                                                ...
                                                                <Tooltip title="Download">
                                                                    <IconButton
                                                                        size="small"
                                                                        onClick={() => downloadImage(doc.imageUrl, `${doc.documentType || 'document'}.jpg`)}
                                                                        disabled={isSubmitting}
                                                                    >
                                                                        <i className="ri-download-line" />
                                                                    </IconButton>
            ...
            <DialogActions sx={{ p: 2.5, pt: 2 }}>
                ...
                <Button
                    ...
                    disabled={isSubmitting || uploadingIndex !== null}
                    startIcon={isSubmitting ? <CircularProgress size={16} color="inherit" /> : null}
                    sx={{ borderRadius: 1.5, px: 4, py: 1, minWidth: 120 }}
                >
                    {isSubmitting ? 'Saving...' : (editData ? 'Update' : 'Save')}
                </Button>
            </DialogActions>
