...
//     // Status change dialog state
//     const [statusDialogOpen, setStatusDialogOpen] = useState(false)
//     const [statusDialogData, setStatusDialogData] = useState({ trip: null, newStatus: '', remarks: '', loading: false })
//     // Form state
...
//         if (viewMode === 'active') {
//             return allData.filter(trip => trip.tripStatus === 'active')
//         }
//         return allData // Show all trips
//     }, [allData, viewMode])
...
//             return (
//                 (row.vehicleNo?.toLowerCase().includes(searchLower)) ||
//                 (row.driverName?.toLowerCase().includes(searchLower)) ||
//                 (row.lhsNo?.toLowerCase().includes(searchLower)) ||
//                 (row.fromLocation?.toLowerCase().includes(searchLower)) ||
//                 (row.toLocation?.toLowerCase().includes(searchLower)) ||
//                 (row.vehicleType?.toLowerCase().includes(searchLower)) ||
//                 (row.routeCode?.toLowerCase().includes(searchLower)) ||
//                 (row.tripStatus?.toLowerCase().includes(searchLower)) ||
//                 (row.accountHolderName?.toLowerCase().includes(searchLower))
//             )
//         })
...
//     const getAvailableVehicles = useMemo(() => {
//         if (!editingItem) {
//             return vehiclesList.filter(vehicle => !getActiveVehicleNumbers.includes(vehicle.vehicleNo))
//         } else {
//             const currentVehicleNo = editingItem.vehicleNo
//             return vehiclesList.filter(
//                 vehicle => vehicle.vehicleNo === currentVehicleNo || !getActiveVehicleNumbers.includes(vehicle.vehicleNo)
//             )
//         }
//     }, [vehiclesList, getActiveVehicleNumbers, editingItem])
//     /* ---------------- HELPER: Check if trip can have status actions ---------------- */
...
//             if (result.success && Array.isArray(result.data)) {
//                 // Filter trips for this specific vehicle only
//                 const vehicleTrips = result.data.filter(trip => trip.vehicleNo === vehicleNo && !trip.isDeleted)
//                 // Filter out current trip if editing
//                 const trips = currentTripId
//                     ? vehicleTrips.filter(trip => trip._id !== currentTripId && trip.id !== currentTripId)
//                     : vehicleTrips
//                 // Count active trips
//                 const activeTrips = trips.filter(trip => trip.tripStatus === 'active')
//                 console.log(`Vehicle ${vehicleNo}: Found ${trips.length} total trips, ${activeTrips.length} active`)
...
//             if (result.success && result.data) {
//                 // Handle both array and single object responses
//                 const vehicle = Array.isArray(result.data) ? result.data.find(v => v.vehicleNo === vehicleNo) : result.data
//                 if (vehicle) {
//                     console.log('Vehicle found:', vehicle)
//                     // Make sure we're updating with the CORRECT vehicle
//                     if (vehicle.vehicleNo === vehicleNo) {
...
//                     route => route.fromLocation?.locationName === fromLocation && route.viaTo?.locationName === toLocation
//                 )
//                 if (matchedRoute) {
//                     setRouteDetails(matchedRoute)
//                     setForm(prev => ({
...
//             updatedForm.totalDieselAmount = calculateTotalDieselAmount(updatedForm.dieselLtr, updatedForm.dieselRate)
//         }
//         setForm(updatedForm)
//     }
//     // ================Add this function to validate trip date=========================================
...
//                 if (tripOnSameDate) {
//                     showSnackbar(
//                         `Vehicle ${vehicleNo} already has a trip on ${selectedDate}. Please select a different date.`,
//                         'warning'
//                     )
//                     return false
//                 }
...
//             setForm(updatedForm)
//             // Handle route changes
//             if ((field === 'fromLocation' || field === 'toLocation') && updatedForm.fromLocation && updatedForm.toLocation) {
//                 fetchRouteDetails(updatedForm.fromLocation, updatedForm.toLocation)
//             }
//             return
//         }
//         // Handle vehicle number change
...
//                 if (!editingItem) {
//                     // Check for trip on same date
//                     const tripOnSameDate = tripCheck.trips.some(trip => trip.tripDate === currentDate)
//                     if (tripOnSameDate) {
//                         showSnackbar(
//                             `Vehicle ${value} already has a trip on ${currentDate}. Please select a different date or vehicle.`,
//                             'warning'
//                         )
//                         setForm(prev => ({ ...prev, vehicleNo: '' }))
//                         setFormLoading(false)
...
//                     if (tripCheck.activeCount >= 2) {
//                         showSnackbar(
//                             `Vehicle ${value} already has ${tripCheck.activeCount} active trips. Maximum 2 allowed.`,
//                             'warning'
//                         )
//                         setForm(prev => ({ ...prev, vehicleNo: '' }))
//                         setFormLoading(false)
...
//                     const tripOnSameDate = tripCheck.trips.some(trip => trip.tripDate === value)
//                     if (tripOnSameDate) {
//                         showSnackbar(
//                             `Vehicle ${form.vehicleNo} already has a trip on ${value}. Please select a different date.`,
//                             'warning'
//                         )
//                         // Revert to previous date
//                         setForm(prev => ({
...
//                         }
//                         : null
//                 setForm({ vehicleNo: trip.vehicleNo || '', driverName: trip.driverName || '', vehicleType: trip.vehicleType || '', driverMobile: trip.driverMobile || '', fromLocation: trip.fromLocation || '', lhsNo: trip.lhsNo || '', toLocation: trip.toLocation || '', routeCode: '', dieselLtr: trip.dieselLtr || '', ifscCode: trip.ifscCode || '', dieselRate: trip.dieselRate || '', accountNo: trip.accountNo || '', totalDieselAmount: trip.totalDieselAmount || '', bankName: trip.bankName || '', advanceAmount: trip.advanceAmount || '', accountHolderName: trip.accountHolderName || '', totalAdvanceAmount: trip.totalAdvanceAmount || trip.advanceAmount || '', tripStatus: trip.tripStatus || 'active', tripDate: trip.tripDate || new Date().toISOString().split('T')[0], routeCode: trip.routeCode || '', selectedRoute: routeObj })
//                 setDialogOpen(true)
//             }
//         } catch (error) {
//             console.error('Error loading trip details:', error)
...
//                 if (tripCheck.activeCount >= 2) {
//                     showSnackbar(
//                         `Vehicle ${form.vehicleNo} already has ${tripCheck.activeCount} active trips. Maximum 2 active trips allowed.`,
//                         'error'
//                     )
//                     setFormLoading(false)
//                     return
...
//                 if (tripOnSameDate) {
//                     showSnackbar(
//                         `Vehicle ${form.vehicleNo} already has a trip on ${form.tripDate}. Please select a different date.`,
//                         'error'
//                     )
//                     setFormLoading(false)
//                     return
...
//             }
//             const submitData = {
//                 vehicleNo: form.vehicleNo, driverName: form.driverName, vehicleType: form.vehicleType, driverMobile: form.driverMobile, fromLocation: form.fromLocation, lhsNo: form.lhsNo, toLocation: form.toLocation, routeCode: form.routeCode, dieselLtr: form.dieselLtr, ifscCode: form.ifscCode, dieselRate: form.dieselRate, accountNo: form.accountNo, totalDieselAmount: form.totalDieselAmount, bankName: form.bankName, advanceAmount: form.advanceAmount, accountHolderName: form.accountHolderName, totalAdvanceAmount: form.totalAdvanceAmount || form.advanceAmount || 0, tripStatus: form.tripStatus, tripDate: form.tripDate
//             }
//             let response, result
//             if (editingItem) {
//                 response = await fetch(TRIPS_API, {
...
//                             />
//                             {balance === 0 && trip.totalAdvanceAmount > 0 && (
//                                 <Chip label='Advance Paid' color='success' size='small' variant='outlined' />
//                             )}
//                         </div>
//                     )
//                 }
...
//                                 <ToggleButton value='active'>Active Trips Only</ToggleButton>
//                                 <ToggleButton value='all'>Show All Trips</ToggleButton>
//                             </ToggleButtonGroup>
...
//                             <TextField
//                                 size='small'
//                                 placeholder='Search vehicle, driver, LHS...'
//                                 value={searchTerm}
...
//                                     {allData.length > 0 ? `There are ${allData.length} closed/cancelled trips.` : 'Create your first trip!'}
//                                 </Typography>
//                             ) : (
//                                 <Typography color='textSecondary'>No trips found. Create your first trip!</Typography>
//                             )}
...
//                                         title="First Page"
//                                     >
//                                         <i className='ri-first-page-line' />
//                                     </IconButton>
//                                     <IconButton
...
//                                     Found {filteredData.length} matching {filteredData.length === 1 ? 'trip' : 'trips'}
//                                 </Typography>
//                             )}
//                         </>
//                     )}
...
//                                 }}
//                                 renderInput={params => (
//                                     <TextField {...params} label='Vehicle No*' required disabled={formLoading} size='small' />
//                                 )}
//                             />
//                             <TextField
//                                 label='Driver Name*'
...
//                             <TextField
//                                 label='Total Advance Amount'
//                                 value={(Number(form.advanceAmount) || 0) + (Number(form.totalDieselAmount) || 0)}
//                                 InputProps={{
//                                     readOnly: true,
//                                     startAdornment: <span className='mr-2'></span>
//                                 }}
...
//                         </Typography>
//                         <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
//                             <RadioGroup row value={form.tripStatus} onChange={e => handleFormChange('tripStatus', e.target.value)}>
//                                 {/* For ADD mode (creating new trip): Show only "Active" */}
//                                 {!editingItem ? (
//                                     <FormControlLabel value='active' control={<Radio />} label='Active' disabled={formLoading} />
//                                 ) : (
//                                     /* For EDIT mode: Show all status options based on current trip status */
//                                     <>
//                                         {form.tripStatus === 'active' && (
...
//                     <Box sx={{ pt: 2 }}>
//                         <Typography variant='body1' gutterBottom>
//                             Trip: <strong>{statusDialogData.trip?.vehicleNo}</strong> - {statusDialogData.trip?.fromLocation} →{' '}
//                             {statusDialogData.trip?.toLocation}
//                         </Typography>
//                         <Typography variant='body2' color='text.secondary' gutterBottom>
//                             Current Status: <strong>{statusDialogData.trip?.tripStatus?.toUpperCase()}</strong>
//                         </Typography>
...
//                 </DialogContent>
//                 <DialogActions>
//                     <Button onClick={closeStatusChangeDialog} disabled={statusDialogData.loading}>
//                         Cancel
//                     </Button>
//                     <Button
//                         variant='contained'
...
//                         startIcon={statusDialogData.loading && <CircularProgress size={16} />}
//                         color={
//                             statusDialogData.newStatus === 'completed'
//                                 ? 'success'
//                                 : statusDialogData.newStatus === 'cancelled'
//                                     ? 'warning'
//                                     : statusDialogData.newStatus === 'closed'
//                                         ? 'info'
//                                         : 'primary'
//                         }
//                     >
        ...
        try {
            ...
            if (result.success && result.data) {
                ...
                setForm({
                    ...
                    accountHolderName: trip.accountHolderName || '',
                    advanceAmount: trip.advanceAmount || '',
                    totalAdvanceAmount: trip.totalAdvanceAmount || trip.advanceAmount || 0,
                    tripStatus: trip.tripStatus || 'active',
                    tripDate: trip.tripDate || new Date().toISOString().split('T')[0]
                })
                setDialogOpen(true)
                    ...
                    <Box sx={{ pt: 2 }}>
                        ...
                        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 2, mb: 3 }}>
                            ...
                            <TextField
                                label='Driver Mobile'
                                value={form.driverMobile}
                                onChange={e => handleFormChange('driverMobile', e.target.value)}
                                disabled={formLoading}
                                size='small'
                        ...
                        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 2, mb: 3 }}>
                            ...
                            <TextField
                                ...
                                onChange={e => handleFormChange('totalAdvanceAmount', e.target.value)}
                                InputProps={{
                                    readOnly: true,
