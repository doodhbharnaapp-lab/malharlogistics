...
//                 // The earliest active trip is the one that should be manageable
//                 const earliestActiveTrip = sortedActiveTrips[0]
//                 // FIX: Use consistent ID field
//                 const earliestActiveTripId = earliestActiveTrip._id || earliestActiveTrip.id
//                 const currentId = currentTripId || ''
//                 // If there's only one active trip for this vehicle, always allow it
//                 if (sortedActiveTrips.length === 1) {
//                     return {
//                         hasOverlapping: false,
//                         message: '',
//                         activeTrips: sortedActiveTrips,
...
//                 }
//                 // If current trip is the earliest active trip, it's valid to manage
//                 if (earliestActiveTripId === currentId) {
//                     return {
//                         hasOverlapping: false,
//                         message: '',
//                         activeTrips: sortedActiveTrips,
...
//                 }
//                 // Check if current trip is among active trips
//                 const isCurrentActive = activeTrips.some(t =>
//                     (t._id || t.id) === currentId
//                 )
//                 if (isCurrentActive && sortedActiveTrips.length > 1) {
//                     // Current trip is active but not the earliest
//                     return {
//                         hasOverlapping: true,
//                         message: `Cannot manage this trip. Vehicle ${vehicleNo} has an EARLIER active trip from ${new Date(earliestActiveTrip.tripDate || earliestActiveTrip.createdAt).toLocaleDateString()} (${earliestActiveTrip.fromLocation || ''} → ${earliestActiveTrip.toLocation || ''}). Please complete or cancel that trip first.`,
//                         activeTrips: sortedActiveTrips,
//                         earliestTrip: earliestActiveTrip,
//                         isEarliest: false
//                     }
...
'use client'
import { useState, useMemo, useRef, useEffect } from 'react'
import { Card, CardContent, Typography, Button, Dialog, DialogTitle, DialogContent, TextField, MenuItem, Divider, IconButton, Table, TableHead, TableRow, TableCell, TableBody, CircularProgress, Alert, Snackbar, Autocomplete, Grid, Box, Tabs, Tab, Paper, FormControl, Select, InputAdornment } from '@mui/material';
import { createColumnHelper, getCoreRowModel, useReactTable, flexRender } from '@tanstack/react-table'
import tableStyles from '@core/styles/table.module.css'
import { DatePicker } from '@mui/x-date-pickers/DatePicker'
import { LocalizationProvider } from '@mui/x-date-pickers/LocalizationProvider'
...
const AdvanceRegister = () => {
    ...
    const [trip, setTrip] = useState({ id: null, _id: null, vehicleNo: '', vehicleType: '', fromLocation: '', toLocation: '', lhsNo: '', ifscCode: '', bankName: '', accountNo: '', accountHolderName: '', driverName: '', driverMobile: '', dieselLtr: '', dieselRate: '', totalDieselAmount: '', totalAdvanceAmount: 0, advanceAmount: 0, advances: [], tripStatus: '', tripDate: '' })
    const [advanceForm, setAdvanceForm] = useState({
        advanceType: '',
        amount: '',
        remark: '',
    ...
    const checkForOverlappingTrips = async (vehicleNo, currentTripId = null, currentTripDate = null) => {
        try {
            ...
            if (result.success) {
                const allTrips = result.data || []
                // CRITICAL FIX: Filter trips by vehicle number manually
                // This ensures we only get trips for the specified vehicle
                const tripsForThisVehicle = allTrips.filter(trip =>
                    trip.vehicleNo === vehicleNo
                )
                console.log('All trips from API:', allTrips.map(t => t.vehicleNo))
                console.log('Filtered trips for vehicle:', tripsForThisVehicle.map(t => t.vehicleNo))
                // IMPORTANT: Filter ONLY trips with status 'active' (case insensitive)
                const activeTrips = tripsForThisVehicle.filter(trip => {
                    const status = (trip.tripStatus || '').toLowerCase()
                    return status === 'active'
                })
                console.log('Active trips for vehicle:', activeTrips)
                if (activeTrips.length === 0) {
                    return { hasOverlapping: false, message: '', activeTrips: [] }
                }
                // Sort active trips by date (ascending)
    ...
    //             // Remove diesel advances completely
    //             const nonDieselUnpaid = unpaidAdvances.filter(
    //                 a => !a.advanceType.toLowerCase().includes('diesel')
    //             )

    //             if (nonDieselUnpaid.length === 0) return false

    //             const totalUnpaid = nonDieselUnpaid.reduce((s, a) => s + Number(a.amount || 0), 0)
        ...
        try {
            ...
            const tableData = rowsWithUnpaid.map((trip, index) => {
                ...
                return [
                    ...
                    trip.toLocation || 'N/A',
                    formatAmount(trip.totalAdvanceAmount || 0),
                    formatAmount(totalAdvancePaid),
                    formatAmount(latestUnpaidAdvance),
                    formatAmount(balance),
                    trip.ifscCode || 'N/A',
                    trip.accountNo || 'N/A',
                    trip.bankName || 'N/A',
                    trip.accountHolderName || 'N/A',
            ...
            if (tableData.length === 0) {
                ...
                doc.setTextColor(100)
                doc.text(
                    'No entries with unpaid advances found',
                    doc.internal.pageSize.width / 2,
                    100,
                    { align: 'center' }
                )
...
`
    ...
    const table = useReactTable({
        data: paginatedRows, // Changed from 'rows' to 'paginatedRows'
        columns,
        getCoreRowModel: getCoreRowModel()
    })
    /* ================= BULK PROCEED FOR SELECTED DATE ================= */
                    ...
                    <CardContent>
                        ...
                        {tabValue === 0 ? (
                            ...
                            <>
                                ...
                                {searchTerm && filteredRows.length === 0 && !loading && (
                                    ...
                                    <>
                                        ...
                                        <div className='flex justify-between items-center mt-4 px-2'>
                                            ...
                                            <div className='flex items-center gap-2'>
                                                ...
                                                <IconButton
                                                    ...
                                                    disabled={page === totalPages || totalPages === 0 || filteredRows.length === 0}
                                                    size='small'
                                                    title="Next Page"
                                                >
                                        ...
                                        {searchTerm && filteredRows.length > 0 && (
                                            <Typography variant='caption' color='textSecondary' sx={{ display: 'block', mt: 1, px: 2 }}>
                                                Found {filteredRows.length} matching {filteredRows.length === 1 ? 'trip' : 'trips'}
                                            </Typography>
                                        )}
                                    </>
                                )}
                    ...
                    <DialogContent dividers>
                        {!trip._id ? (
                            ...
                            <div className='flex flex-col gap-4'>
                                ...
                                <Autocomplete
                                    options={filteredRows.filter(
                                        row =>
                                            row.vehicleNo &&
                                            row.availableBalance > 0 &&
                                            (row.tripStatus === 'active' || row.tripStatus === 'Active') &&
                                            row.canManageAdvances === true
                                    )}
                                    getOptionLabel={option =>
                                        `${option.vehicleNo} - ${option.driverName || ''} (Available: ${option.availableBalance.toFixed(2)})`
                                    }
